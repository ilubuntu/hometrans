...
| MCP | 谁在用 | 作用 | 调用时机 |
|---|---|---|---|
| `homegraph` | `spec-generator` | 作为代码分析服务读取 Android 源工程，建立代码索引；用于把人工需求对应到真实入口、调用链、状态和数据流，支撑 SPEC 生成 | spec-generate 阶段（Step 2 建索引 + Step 3 逐个 REQ 查代码） |
...
代码分析服务：HomeTrans 在生成 SPEC 时会调用它读取 Android 源码；新版本统一使用内置的 homegraph（旧版曾用 gitnexus，已移除）。
hometrans MCP：HomeTrans 里的一个辅助 MCP，主要服务 pipeline 的代码评审阶段。
```

所以前后关系是：HomeTrans 调度迁移流程，内置的 homegraph 帮它理解 Android 源码，`hometrans` MCP 帮它在评审阶段缩小 HarmonyOS 改动检查范围。homegraph 是 HomeTrans 自带的代码分析后端，不是与 HomeTrans 同级的独立系统（gitnexus 已在新版本中移除）。
...
| 代码分析服务（homegraph） | 分析 Android 代码结构，支撑 SPEC 生成 | `hmos-spec-generate` 不能正常工作 |
...
第 2 步会让 homegraph 扫描 Android 工程，生成代码索引。后面“根据需求找入口、查调用链、定位数据流”都依赖这个索引；如果源码变过，索引也要刷新。
...
| 2 | 调用 homegraph 扫描 Android 源码，建立或刷新后续查询用的代码索引 | homegraph 代码分析能力 | 可查询的代码知识库 |
| 3.1 | 读取单个 REQ，提取页面、用户动作、业务对象、边界条件 | Agent 按 skill workflow 分析需求 | 需求要点 |
| 3.2 | 顺着需求关键词和调用关系，找到用户触发这个需求的入口 | homegraph 查询 + Agent 判断 | 入口候选 |
| 3.3 | 继续查入口背后的点击事件、状态变化、数据读写 | homegraph 查询 + Agent 追踪代码 | 代码证据文件 |
