...
| 工具 | 用途 |
|---|---|
| `testcases_tool.py` | 把 `_extracted.json` 转成 `testcases.json`，并校验格式 |
| `self_test_runner.py` | 安装 HAP、启动 `AutoTest.batch`、查询状态 |
| `report_tool.py` | 根据 AutoTest 原始产物生成 `self-test-report.md` |
| `AutoTest.batch` | 真正执行设备端 UI 用例的 Python 程序入口 |

这里可以顺便说明 AutoTest 的本质：

```text
AutoTest.batch = Python 测试运行器 + 多模态模型 + HDC/Hypium 设备操作能力
```

### 2.5 MCP 服务：Agent 的外部能力

MCP 服务是常驻后台、跨 Agent 共享的基础设施。Agent 在执行过程中按需调用，获取代码分析或上下文切片能力。

| MCP | 谁在用 | 作用 |
|---|---|---|
| `gitnexus` | `spec-generator` | 分析 Android 工程代码结构（调用图、影响面、符号关系），把人工需求对应到真实代码入口和数据流，支撑 SPEC 生成 |
| `hometrans` | `code-reviewer` | 对 logic-coder 的 commit 做 ArkTS 语义切片（改动行 + 调用链上下游 + 资源依赖），告诉 reviewer 该读哪些文件的哪些行，避免全量读文件爆 context |

注意：

```text
gitnexus 是 npm 包 homegraph 提供的 CLI 和 MCP 服务名。
安装命令：npm install -g homegraph arkanalyzer
gitnexus 只分析 Android 源工程，理解原始实现。
hometrans 只分析 HarmonyOS 目标工程，检查转换结果。
两者一进一出，不交叉。
```

hometrans MCP 只在 pipeline Stage 3 评审的第一轮调用一次，针对 logic-coder 的 commit 做精准切片。后续 fixer 的修改不在 MCP 覆盖范围内，靠整体走查兜底。
...
| homegraph（gitnexus） | 分析 Android 代码结构，支撑 SPEC 生成 | `hmos-spec-generate` 不能正常工作 |
...
![HomeTrans 迁移流程](HomeTrans/resource/migration_process.svg)
...
| `ui_info_root` | 可选 | 页面快照目录；不提供时会尝试通过 adb 自动抓取 |
...
0. 检查 DevEco 环境变量
1. 解析用户输入路径
2. 调用 hmos-resources-convert 转换资源
3. 自动探索 Android 页面，生成页面快照
4. 发现要处理的 page_* 页面目录
5. 逐页转换为 ArkTS 页面
6. 统一构建修复
7. 输出批量转换总结
...
| 步骤 | 做什么 | 主要产物 |
|---|---|---|
| 0 | 检查 `DEVECO_HOME` 或 `DEVECO_SDK_HOME`，后面统一构建要用 | 环境是否可继续 |
| 1 | 从命令里解析 Android 工程、Harmony 工程、页面快照目录 | 明确输入路径 |
| 2 | 内部调用 `hmos-resources-convert`，把 Android 资源转到 HarmonyOS | `resource_mapping.md`、`resource_conversion_report.md`、HarmonyOS `resources/` |
| 3 | 如果没有现成页面快照，用 `android_parse_fast.py` 连接 Android 设备自动遍历页面 | `page_NNNN_*/screenshot.png`、`view.xml`、`meta.json`、`index.json`、`report.html` |
| 4 | 扫描 `ui_info_root` 下的 `page_*_*` 目录，确定要迁移哪些页面 | 页面任务列表 |
| 5 | 每个页面交给子 Agent 转换，按 Android view tree、截图和映射规则生成 ArkTS | `entry/src/main/ets/pages/*`、`components/*`、`viewmodel/*` 等 |
| 6 | 所有页面转换完后，只统一调用一次构建修复 | 可构建的 HarmonyOS 工程 |
| 7 | 汇总页面转换结果和遗留 TODO | batch 转换总结 |
...
```text
index.json、page_0001_*/screenshot.png、page_0001_*/view.xml、resource_mapping.md、resource_conversion_report.md
```

资源转换由 `hmos-batch-ui-align` 内部调用；如果只想单独迁移资源，也可以直接使用 `hmos-resources-convert`，见第 5.1 节。
...
| 步骤 | 做什么 | 主要产物 |
|---|---|---|
| 0 | 读取 `android_project_dir`、`harmony_project_dir`、`capture_output_dir`，检查 SDK 路径 | 输入和环境确认 |
| 0.1 | 从 Android/HarmonyOS 工程自动解析 app label 和 package/bundleName | 两端启动信息 |
| 1.1 | 根据用户描述确定要采集的页面路径 | capture plan |
| 1.2 | 操作 Android 和 HarmonyOS 设备进入页面并截图、抓 view tree | `android_page_*`、`hmos_page_*` |
| 1.3 | 发现 tab、弹窗、下拉、展开收起等交互状态并继续采集 | 更多页面状态目录 |
| 2.1 | 分析 Android 页面组件、坐标、颜色、文字、图标、尺寸 | `android_page_*/UI_Analysis.md` |
| 2.2 | 分析 HarmonyOS 页面；采不到时改读 ArkTS 源码分析 | `hmos_page_*/UI_Analysis.md` 或 `UI_Analysis_from_code.md` |
| 2.3 | 对比两端组件差异 | `hmos_page_*/UI_comparison.md` |
| 2.4 | 检查分析文件和对比表是否齐全 | 输出完整性确认 |
| 3 | 把所有差异汇总成修复清单并修改代码 | `fix_checklist.md`、ArkTS 改动 |
| 4 | 调用构建修复验证工程可编译 | 构建结果 |
...
```text
0. 读取 requirements.txt，并按空行拆成多个 REQ
1. 检查 Android 工程是否能被 GitNexus 识别
2. 扫描 Android 源码，建立可查询的代码索引
3. 逐个 REQ 查代码：找用户入口、调用链、状态和数据流
4. 先写代码证据文件，再生成平台无关 SPEC
5. 输出 spec_generation_report.md 汇总报告
```

第 1 步要求 Android 工程在 Git 仓库里，不是为了提交代码，而是为了让 GitNexus 能给这份源码建立稳定身份，后面查询符号、文件和行号时不会串到别的工程。
...
| 步骤 | 做什么 | 主要产物 |
|---|---|---|
| 0 | 读取 `requirements.txt`，按空行拆成多个 REQ，并给每个 REQ 起一个输出文件名 | REQ 列表 |
| 1 | 检查 Android 工程是否能被 GitNexus 正确识别 | 可追踪的源码对象 |
| 2 | 扫描 Android 源码，建立或刷新后续查询用的代码索引 | 可查询的代码知识库 |
| 3.1 | 读取单个 REQ，提取页面、用户动作、业务对象、边界条件 | 需求要点 |
| 3.2 | 顺着需求关键词和调用关系，找到用户触发这个需求的入口 | 入口候选 |
| 3.3 | 继续查入口背后的点击事件、状态变化、数据读写 | 代码证据文件 |
| 3.4 | 检查证据文件是否有遗漏，文件行号是否真实可查 | 可信的代码证据 |
| 3.5 | 根据代码证据生成平台无关 SPEC | `<feature>-SPEC.md` |
| 4 | 汇总生成成功、跳过、失败的 REQ | `spec_generation_report.md` |
...
参数解析和环境检查
  ↓
Stage 1  logic-context-builder：生成逻辑实现计划
  ↓
Stage 1a logic-coder：写 ArkTS 业务逻辑
  ↓
Stage 2  build-fixer：构建 HAP，修复编译错误
  ↓
Stage 3  code-reviewer：对照 SPEC 做代码评审
  ↓
Stage 3a review-fixer：修复确认的问题
  ↓
Stage 3b build-fixer：评审修复后重建
  ↓
Stage 4  self-tester：安装 HAP，跑设备自测
  ↓
Stage 4a self-test-fixer：根据自测失败报告修复
  ↓
Stage 4b build-fixer：自测修复后重建
  ↓
最终汇总 pipeline-manifest.md
```
...
| 阶段 | Agent/能力 | 做什么 | 主要产物 |
|---|---|---|---|
| Stage 1 | `logic-context-builder` | 读取 SPEC 和 HarmonyOS 工程，生成实现计划 | `logic/plan.md` |
| Stage 1a | `logic-coder` | 按计划写 ArkTS 业务逻辑 | ArkTS 代码、`commit-info.md` |
| Stage 2 | `build-fixer` | 构建 signed HAP，循环修复编译错误 | `build-fix-report.md`、`.hap` |
| Stage 3 | `code-reviewer` | 对照 SPEC 检查实现是否覆盖需求 | `review-round-N/code-review-report.md` |
| Stage 3a | `review-fixer` | 从代码角度确认评审问题并修复 | `review-fix-report.md` |
| Stage 3b | `build-fixer` | 评审修复后重新构建 | 新 HAP、构建报告 |
| Stage 4 | `self-tester` | 安装 HAP，解析用例，跑 AutoTest | `self-test-report.md`、`testcases.json`、`task/` |
| Stage 4a | `self-test-fixer` | 根据自测失败报告，从代码角度确认并修复 | `self-test-fix-report.md` |
| Stage 4b | `build-fixer` | 自测修复后重新构建，下一轮测试用新 HAP | 新 HAP、构建报告 |
...
检查输入和 HarmonyOS 工程
  ↓
优先尝试构建或使用 Android APK
  ↓
有 APK 时用 apktool 反编译资源
  ↓
没有 APK 时退回读取源码 res/
  ↓
转换 strings、colors、dimensions、images 等资源
  ↓
检查资源依赖和缺失项
  ↓
生成 resource_mapping.md 和 resource_conversion_report.md
```
...
环境检查
  ↓
解析 test_case.md、signed HAP、output_path
  ↓
询问是否启用自动修复和重测
  ↓
Round 1 self-tester：解析用例、安装 HAP、运行 AutoTest
  ↓
如果失败且启用修复：self-test-fixer
  ↓
build-fixer 重新构建 signed HAP
  ↓
下一轮用新 HAP 重测
  ↓
汇总 self-test-report.md
```
